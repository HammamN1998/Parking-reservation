...
class SignupViewModel extends FormViewModel {
  ...
  bool get typeHasError => _typeHasError;
  set typeHasError(bool value) {
    _typeHasError = value;
    notifyListeners();
  }
  ...
  void validateForm() async {
    ...
    String type = formValueMap[typeValueKey];
    ...
    if (!email.contains('@')) {
      ...
      passwordHasError = true;
    } else if (type != 'normal' && type != 'golden') {
      setValidationMessage('type must be normal or golden');
      typeHasError = true;
    } else {
      try {
        await _auth.signup(email: email, password: password, phone: phone, type: type);
